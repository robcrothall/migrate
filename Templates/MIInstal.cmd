/* Code   */
 ...
 call sysloadfuncs
/*
  Create a log file, and log everything
  Find out what drives exist, and which drive and directory has source
  Offer to browse and/or print the installation instructions
    Drive selection
    Configuring the second copy of Word Perfect
    Ask for the Company name for INI file?
  Check space on each drive and choose one with most free space
  Ask User to confirm target drive selection
  Find/Create directories and save to INI file
    CMD
    MI
    MIProd
    MIBackUp
    BILL
  Copy existing versions of files to a backup directory
  Copy .CMD files to hard drive
    MIGen
    MIGenxx
    MILocGen (remember last filename in INI, give lookup facility)
    VirCheck --> MAX
    MAGCopy
    Queues
    QQuit
  Copy .ICO to CMD directory
  Copy checklists to MAX\FILE\GEN
  Copy category files, MIWords, MIRep
  Copy 'upload' message to MAX/MISC
  Copy Word Perfect files to a special directory
  Copy Word Perfect macros to the correct directory
  Associate icons with commands
  Create folders
  Create program icons within folders
  Read in MAX.CTL and check for VirCheck
  If VirCheck is commented then
    Copy MAX.CTL to backup
    Remove '%' from VirCheck line
    Write new MAX.CTL
  Check that the necessary directories are in PATH in CONFIG.SYS
  Find keyword for Management Insight in MIWords and offer to change it
  Save Company name in INI



  In generator, remember details of last subject
*/
   /* Code    */
